# PostgrustSQL — PostgreSQL-совместимая система управления базами данных на Rust
...
PostgrustSQL — это реляционная СУБД, написанная на Rust с PostgreSQL-совместимым протоколом подключения. Поддерживает расширенные SQL-операции, MVCC, транзакции, индексы, по умолчанию работает на порту 5432.

## 🚀 Быстрый старт

> **📖 Полная инструкция по установке и настройке:** [INSTALL.md](INSTALL.md)
>
> Включает: установку из исходников, пакеты для Arch/Debian, настройку конфигурации, systemd сервис, переменные окружения, и troubleshooting.
...
- **Составные индексы**: поддержка мультистолбцовых индексов
...
- `VARCHAR(n)` — ограничение длины с проверкой
- `CHAR(n)` — фиксированная длина (с дополняющими пробелами)
...
PostgrustSQL поддерживает транзакции с изоляцией снимков (snapshot isolation):
...
**Важно:** Изменения в транзакции видны сразу, но записываются в подсистему хранения только после COMMIT.
...
- **Rust Edition 2024**
- **tokio 1.41** — асинхронная среда выполнения
- **nom 7.1** — парсер комбинаторы для SQL
- **serde 1.0 + bincode 1.3** — бинарная сериализация (журналы предзаписи, снимки)
- **serde_json 1.0** — поддержка JSON/JSONB 
- **thiserror 2.0** — обработка ошибок
- **comfy-table 7.1** — красивое форматирование таблиц
- **rustyline 14.0** — CLI с историей команд
- **chrono 0.4** — типы Date/Time
- **uuid 1.6** — UUID-тип
- **rust_decimal 1.33** — тип NUMERIC с точностью
